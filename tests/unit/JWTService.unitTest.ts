...
describe("JWTService", () => {
  context("when no valid roles returned", () => {
    it("should return false", () => {
      ...
      expect(jwtService.isAtLeastOneRoleValid(decodedToken)).toBeFalsy();
    });
  });
    ...
    context("with missing config items", () => {
      context("missing all", () => {
        it("it throws AuthorisationError", async () => {
          ...
          expect.assertions(2);
          try {
            await srv.verify("abc123", mockConf);
          } catch (e) {
  ...
  context("with AtLeastOneValidRole = true", () => {
    it("invokes fetchJWK", async () => {
      ...
      const output = await srv.verify(myToken, mockConf);
  ...
  context("with AtLeastOneValidRole = false", () => {
    it("it throws AuthorisationError", async () => {
      ...
      JWTService.prototype.fetchJWK = jest.fn().mockResolvedValue("A_CERTIFICATE");
      JWTService.prototype.isAtLeastOneRoleValid = jest.fn().mockReturnValue(false);
      const srv = new JWTService();
      const myToken = "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsImtpZCI6IjFMVE16YWtpaGlSbGFfOHoyQkVKVlhlV01xbyJ9..1AFWW-Ck5nROwSlltm7GzZvDwUkqvhSQpm55TQsmVo9Y59cLhRXpvB8n-55HCr9Z6G_31_UbeUkoz612I2j_Sm9FFShSDDjoaLQr54CreGIJvjtmS3EkK9a7SJBbcpL1MpUtlfygow39tFjY7EVNW9plWUvRrTgVk7lYLprvfzw-CIqw3gHC-T7IK_m_xkr08INERBtaecwhTeN4chPC4W3jdmw_lIxzC48YoQ0dB1L9-ImX98Egypfrlbm0IBL5spFzL6JDZIRRJOu8vecJvj1mq-IUhGt0MacxX8jdxYLP-KUu2d9MbNKpCKJuZ7p8gwTL5B7NlUdh_dmSviPWrw";
      expect.assertions(2);
      try {
        await srv.verify(myToken, mockConf);
      } catch (e) {
        expect(e).toBeInstanceOf(AuthorizationError);
